...
\begin{array}{rcll}
	...
	\partial_t\Li{x} & = & \left\lbrack k_x^l\Theta'_x+\omega_x E_0\right\rbrack
		...
		\right\rbrack & \\
	\beta & = & \Theta + \rho_h \left(1-\alpha_\infty\right) & \rho_h = \dfrac{\bar{p}^2 \proton E_0}{1+J_6\LiOut{6}+J_7\LiOut{7}} \\
	\end{array}
\right.
\end{equation}
and using
...
\begin{array}{rcl}
	...
	\left[
	 ...
	 {k_x^l-\omega_h}
	 \right]\\
	 & = & \beta \left[1-e^{-k_x^l t}\right] + \gamma_h \Xi\left(k_x^lt,\sigma_x\right)\\
	 \\
	 \sigma_x & = & \dfrac{\omega_x}{k_x^l}\\
	 \\
	 \gamma_h & = & \rho_h \alpha_\infty \\
	 \\
...
\begin{array}{rl}
	\Li{6} + \Li{7} & = \beta \left( \left[1-e^{-k_6^l t}  \right] \LiOut{6} + \left[1-e^{-k_7^l t}  \right] \LiOut{7} \right)\\
	 & + \gamma_h \left(\Xi\left(k_6^lt,\sigma_6\right) \LiOut{6} +  \right)\\
\end{array}
\end{equation}
