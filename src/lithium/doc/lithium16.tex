...
and we set
\begin{equation}
	\Upsilon_x = \bar{p}_x^2 \dfrac{\LiAllOut}{1+J_\epsilon \LiAllOut} \; \text{ in M}^{-1} , \;\; Q_x = \bar{q}_x \LiAllOut \text{ as scalar}
\end{equation}
to get
\begin{equation}
\left\lbrace
\begin{array}{rcl}
	...
	\partial_t \hat\alpha & = & k_h - \hat\alpha \left\lbrack k_h+
	 \proton \left(\sum_x \epsilon_x k_x \Upsilon_x \right) \right] 
		+ (1-\hat\alpha) \left\lbrack {\sum_x \epsilon_x k_x Q_x \beta_x }  \right\rbrack \LiAllOut \\
\end{array}
\right.
\end{equation}
