...
Experimentally, the initial isotopic separation is eventually recovered, meaning that
\begin{equation}
	\beta_6(\infty) = \beta_7(\infty) = \beta_\infty =  \dfrac{k_x\Theta+ \left(1-\alpha_\infty\right) h_\infty \dfrac{k_x^p \epsilon_x J_x E_0 }{1+\left(\sum_x J_x \epsilon_x \right) \LiAllOut} }{ k_x + \alpha_\infty k_x^q \epsilon_x E_0 }
\end{equation}
And then formally, we simply have the other equation
\begin{equation}
	k_h E_0 \alpha_\infty =  (k_6+k_7) \left(\beta_\infty-\Theta\right) \LiAllOut
\end{equation}
