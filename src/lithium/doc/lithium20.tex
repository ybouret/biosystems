...
\item We replace $Q_x$ by its value to solve $\alpha_\infty$ using
\begin{equation}
	\hat\alpha_\infty = 
	\dfrac{1
	+\dfrac{1}{\eta(1-\hat\alpha_\infty)}\left[ \left(\sum_x \epsilon_x \mu_x\left[\Theta-\beta_x\right]\right) 
		+\eta \hat\alpha_\infty \left(\sum_x \epsilon_x h_\infty \Upsilon_x\right) \right]
		}
	{1+\left(\sum_x \epsilon_x h_\infty \Upsilon_x\right) + \dfrac{1}{\eta(1-\hat\alpha_\infty)}\left[ \left(\sum_x \epsilon_x \mu_x\left[\Theta-\beta_x\right]\right) 
		+\eta \hat\alpha_\infty \left(\sum_x \epsilon_x h_\infty \Upsilon_x\right) \right]
}
\end{equation}
We formally use
\begin{equation}
	G = \dfrac{\sum_x \epsilon_x \mu_x\left[\Theta-\beta_x\right]}{\eta}, \;\;A=\sum_x \epsilon_x h_\infty \Upsilon_x = \dfrac{h_\infty}{h_0} \tan^2\theta_0
\end{equation}
to get
\begin{equation}
	\hat\alpha_\infty = \dfrac{1
	+\dfrac{1}{\eta(1-\hat\alpha_\infty)}\left[\eta G+\eta \hat\alpha_\infty A \right]}
	{1+A+\dfrac{1}{\eta(1-\hat\alpha_\infty)}\left[G+\eta \hat\alpha_\infty A \right]}
	= \dfrac{
	(1-\hat\alpha_\infty) + \left[ G+ \hat\alpha_\infty A \right]
	}
	{
	\left(1+A\right)(1-\hat\alpha_\infty) + \left[ G+ \hat\alpha_\infty A \right]
	}
\end{equation}
An we find
\begin{equation}
	\hat\alpha_\infty = 1+G \;\;(\Rightarrow -1 < G < 0)
\end{equation}
\end{itemize}
