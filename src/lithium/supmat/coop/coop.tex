...
\\
& = & \mathsf{E}\proton_0 \dfrac{\mathsf{N}_x(\LiAllOut)}{1+\mathsf{D}_x(\LiAllOut)} + k_x \Theta \\
\end{array}
\right.
\end{equation}
where 
\begin{itemize}
\item ${\mathsf{N}_x}$ is a {\bf second} order polynomial without constant term
\item ${\mathsf{D}_x}$ is a {\bf third } order polynomial without constant term
\end{itemize}
