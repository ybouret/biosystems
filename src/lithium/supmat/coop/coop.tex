...
\newcommand{\mycolor}[2]{\ifthenelse{\equal{#1}{6}}{{\color{Orange}#2}}{{\color{Cerulean}#2}}}
...
and
\begin{equation}
\mymat{Q}_7 = \dfrac{i_7 u_7^3}{1+D}
\begin{bmatrix}
	D^2 \kappa^2 & D\kappa \\
	D\kappa      & 1\\
\end{bmatrix}
= \dfrac{i_7 u_7^3}{1+D} \vec{L} \mytrn{\vec{L}},\;\; \mymat{Q_6} = \kappa \mymat{Q}_7
\end{equation}
so that
\begin{equation}
\left\lbrace
...
\\
 & = & i_7 u_7^2  <\vec{L}|\vec{C}> \left[ 1 + \dfrac{u_7}{1+D}  <\vec{L}|\vec{C}> \right]\\
 \\
 & = & i_7 \left[  u_7^2  <\vec{L}|\vec{C}> + \dfrac{u_7^3}{1+D} <\vec{L}|\vec{C}>^2 \right]\\
 \\
...
\mymat{Q}_d & =  &
\begin{bmatrix}
(2D+1)\kappa^2 & (D+1)\kappa \\
(D+1) \kappa   & (D+2)  \\
\end{bmatrix}\\
\\
T_d & = & C_7^3 + (1+D)[C_7^2(\kappa C_6)+C_7(\kappa C_6)^2] + D (\kappa C_6)^3 \\
\end{array}
\right.
\end{equation}
we get
\begin{equation}
	\mathsf{D} = \dfrac{1}{1+D} \left[ u_7 <\vec{C}|\vec{L}_d> + u_7^2 <\vec{C}|\mymat{Q}_d|\vec{C}> + u_7^3 T_d \right]
\end{equation}
