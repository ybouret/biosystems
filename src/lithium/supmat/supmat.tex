...
\label{eq:sigma}
	\sigma  =  \dfrac{k_6}{k_7}.
\end{equation}
The GHK theory states that a passive-like behaviour is expected through the ion channel that lithium isotopes will cross.
We hence expect the constant rates ($k_x$) to be in the same order of magnitude, so we assume \todo{ref}:
\begin{equation}
	\sigma \simeq 1.00229,
\end{equation}
...
\end{equation}
which is far greater than the observed values even after some time \todo{ref figures}
...
\section{Parameters Estimation from Passive-Only Experiments}
The catalytic pathway can be simply discarded through two conditions:
\begin{itemize}
	\item we block $\NHE{1}$ by keeping the acidic medium,
	\item we use   $PS120$ cells lacking $\NHE{1}$.
\end{itemize}
In both cases, we shall observe 
